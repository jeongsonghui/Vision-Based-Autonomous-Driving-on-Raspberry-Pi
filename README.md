...
본 프로젝트는 수업에서 제공된 기본 자율주행 코드를 기반으로 개발되었습니다.  
해당 코드를 그대로 사용하는 것이 아니라, 실제 주행 성능 개선을 위해 수정 및 확장을 수행하였습니다.

- ROI, threshold 등 전처리 파라미터를 모델 출력 및 주행 환경에 맞게 조정
- 조향각 범위 재설정 및 제어 로직 개선
- CNN / NVIDIA 모델을 활용한 추론 구조 개선
- 횡단보도 인식 후 정지 및 재출발 기능 적용
